...
test("roving tab focus survives a delayed render", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const { tabs } = await import("/js/ui.js?v=20260921-report-drive-r1");
...
test("tab helper creates stable tab-to-panel relationships", async ({ page }) => {
  await openAuth(page);
  await page.evaluate(async () => {
    const { tabIds, tabPanel, tabs } = await import("/js/ui.js?v=20260921-report-drive-r1");
...
test("production student tabs control a labelled panel", async ({ page }) => {
  await loginAs(page, "student");
  await page.evaluate(async () => {
    const { state } = await import("/js/state.js?v=20260921-report-drive-r1");
    const { render } = await import("/js/renderer.js?v=20260921-report-drive-r1");
    state.view = "mine";
