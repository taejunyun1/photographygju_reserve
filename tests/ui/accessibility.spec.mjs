...
test("roving tab focus survives a delayed render", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const { tabs } = await import("/js/ui.js?v=20260923-reservation-requests-r1");
...
test("tab helper creates stable tab-to-panel relationships", async ({ page }) => {
  await openAuth(page);
  await page.evaluate(async () => {
    const { tabIds, tabPanel, tabs } = await import("/js/ui.js?v=20260923-reservation-requests-r1");
