...
- 종류별 예약 가능 자원과 최근 28일을 곱한 예약 가능 횟수를 분모로 사용한다. 기자재는 활성·예약 가능·상태 `가능`인 장비 수, 스튜디오는 설정된 공간 수, 암실은 설정 정원, 출력실은 설정 시간창 정원을 사용한다.
- 예약 가능 횟수가 3회 미만인 종류·시간은 데이터 부족으로 제외한다.
- 상위 3개와 `유효 예약 수 / 예약 가능 횟수` 점유율을 표시한다.

장비 가동률:

- 최근 28일 동안 각 활성·예약 가능 장비가 예약된 서로 다른 일수 ÷ 28 × 100
- 수리중·문의 전용 장비는 제외하고, 사용 이력이 있는 상위 5개만 표시한다. 화면 문구는 실제 반납 완료 사용이 아니라 예약된 날짜 기준임을 명시한다.

취소율:

- 최근 28일에 접수된 요청(`createdAt`, 구형 데이터는 예약일로 보완)의 취소·관리자 취소·반려 요청 수 ÷ 같은 기간 전체 요청 수 × 100
- 전체 요청이 5건 미만이면 카드 대신 데이터 부족 상태로 표시한다.
