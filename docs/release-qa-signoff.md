...
## 2026-07-29 기자재 운영 확장 QA

| 점검 | 결과 | 메모 |
| --- | --- | --- |
| 재고 코드 형상 | 통과 | 전체 144개, 신규 코드 144개, 중복 0개, 미해결 제품키 0개 |
| `npm run release:check` | 통과 | 기자재 코드·반납 점검 포함 전체 도메인·보안·저장소 검사 |
| Playwright | 통과 | 118 통과, 조건부 14 건너뜀, 실패 0 |
| 신규 관리자 UI | 통과 | 자동 코드·전체 재발급·장비별 반납 점검을 390/430/768/1440px에서 검증 |
| `npm run build` | 통과 | React 관리자·학생 번들과 `dist/` 생성 |
| `npm run pages:check` | 통과 | Cloudflare Pages 정적 배포 준비 확인 |
| `npm run deploy:check` | 통과 | 현재 운영 Worker 상태와 필수 보안 계약 확인 |
| `npm run native:release:check` | 조건부 통과 | 네이티브 구성 통과, Play AAB 빌드 전 upload-key 환경변수 필요 |
| `git diff --check` | 통과 | 공백 오류 없음 |

검증한 기자재 운영 계약:

- 신규 코드는 `분류-브랜드-제품키-3자리 일련번호`이며 `codeVersion: 2`다.
- 기존 코드는 `legacyCodes`에 보존하고 관리자 검색에서만 사용한다.
- 전체 재발급은 미리보기 지문과 경고 확인 후 원자적으로 적용한다.
- 대여 중 예약은 장비별 정상·점검 필요·수리 필요 확인 없이 바로 반납 완료할 수 없다.
- 점검·수리 결과는 장비 가용 상태와 검사 이력에 함께 반영한다.
